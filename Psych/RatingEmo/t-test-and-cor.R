...
data.combined <- data.frame(combinedForSum1$hpa1b8,combinedForSum1$hpa2b8,combinedForSum1$ssa1b8,combinedForSum1$ssa2b8)
data.combined <- as.data.frame(t(data.combined))
# set vectors for labelling the data with location and pollutant
TD = c('keyboard', 'Remote', 'keyboard', 'Remote')
per = c('Partner', 'Partner', 'Self', 'Self')

# add locations and pollutants to the data
data.combined$TD = TD
data.combined$per = per
...
stacked.data = melt(data.combined, id = c('TD', 'per'))
# remove the column that gives the column name of the concentration from all.data
stacked.data = stacked.data[, -3]
p <- ggplot(data = stacked.data, aes(x=per, y=value)) + geom_boxplot(aes(fill=TD))
p <- p + xlab("Person") + ylab("Score") + ggtitle("Interest")
print (p)
